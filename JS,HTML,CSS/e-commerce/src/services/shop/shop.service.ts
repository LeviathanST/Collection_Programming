...
import { updateShopDto } from "src/dtos/shop.dto";
...
export class ShopService {
    ...
    async updateShop(authToken: string, shopDetails: updateShopDto) {
        const validator = ValidationUtils(["shop_name", "shop_thumb", "shop_id"], shopDetails);
        ...
        const getShop = await this.shopRepository.findOneBy({ shop_id: shopDetails.shop_id })
            .then(data => data ? data : undefined)
            .catch(err => { throw err });

        if (!getShop) {
            throw new NotFoundException("Shop is not existed!");
        }

        if (verify.username !== getShop.shop_owner) {
            throw new BadRequestException("You is not enough permission!");
        }

        return this.shopRepository.update({ shop_id: shopDetails.shop_id }, shopDetails);
    }
